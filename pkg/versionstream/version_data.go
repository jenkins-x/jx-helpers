...
func (data *StableVersion) VerifyPackage(name string, currentVersion string, workDir string, warnOnMissing bool) error {
	...
	if version == "" && warnOnMissing {
		log.Logger().Warnf("could not find a stable package version for %s from %s\nFor background see: https://jayex.io/v3/about/concepts/version-stream/", name, workDir)
...
func LoadStableVersionNumber(wrkDir string, kind VersionKind, name string, warnOnMissing bool) (string, error) {
	...
	if version != "" {
		log.Logger().Debugf("using stable version %s from %s of %s from %s", termcolor.ColorInfo(version), string(kind), termcolor.ColorInfo(name), wrkDir)
	} else {
		...
		if warnOnMissing {
			log.Logger().Warnf("could not find a stable version from %s of %s from %s\nFor background see: https://jayex.io/about/concepts/version-stream/", string(kind), name, wrkDir)
...
func ResolveDockerImage(versionsDir, image string) (string, error) {
	...
	if info.Version == "" {
		...
		log.Logger().Warn("for background see: https://jayex.io/about/concepts/version-stream/")
