...
func AppendMyValues(valueFiles []string) ([]string, error) {
	...
	if exists {
		...
		log.Logger().Infof("Using local value overrides file %s", termcolor.ColorInfo(myValuesFile))
	} else {
...
func InspectChart(chart string, version string, repo string, username string, password string,
	...
	dirPrefix := fmt.Sprintf("jx-helm-fetch-%s-", files.ToValidFileSystemName(chart))
	if isLocal {
