...
func TestReplaceAllStringSubmatchFunc(t *testing.T) {
	incrementVersionRepl := func(groups []stringhelpers.Group) []string {
	...
	brewVersionRepl := func(groups []stringhelpers.Group) []string {
		answer := make([]string, 0)
		answer = append(answer, "1.0.20")
		return answer
	}
	brewShaRepl := func(groups []stringhelpers.Group) []string {
		answer := make([]string, 0)
	...
	type args struct {
		re   *regexp.Regexp
		str  string
		repl func([]stringhelpers.Group) []string
	}
	tests := []struct {
		name string
		args args
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := stringhelpers.ReplaceAllStringSubmatchFunc(tt.args.re, tt.args.str, tt.args.repl); got != tt.want {
