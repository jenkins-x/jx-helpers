...
func TestCreateOrUpdateActivities(t *testing.T) {
	...
	key := activities.PipelineActivityKey{
		Name:     expectedName,
	...
	promoteStarted := func(a *v1.PipelineActivity, s *v1.PipelineActivityStep, ps *v1.PromoteActivityStep, p *v1.PromoteUpdateStep) error {
		assert.NotNil(t, a)
		assert.NotNil(t, p)
		activities.CompletePromotionUpdate(a, s, ps, p)
		return nil
	}
...
func TestCreatePipelineDetails(t *testing.T) {
	...
	for _, pipeline := range pipelines {
		d1 := activities.CreatePipelineDetails(pipeline)
		name := pipeline.Name
...
func TestPipelineID(t *testing.T) {
	...
	pID = activities.NewPipelineID("O/N!R@1", "therepo", "thebranch")
...
func TestSortActivities(t *testing.T) {
	...
	activities.SortActivities(acts)

	assert.Equal(t, "a3", acts[0].Name, "Activity 0")
	assert.Equal(t, "a2", acts[1].Name, "Activity 1")
	assert.Equal(t, "a1", acts[2].Name, "Activity 2")
	assert.Equal(t, "a4", acts[3].Name, "Activity 3")
}
...
func TestSortActivitiesWithPendingCases(t *testing.T) {
	...
	assert.Equal(t, "a2", acts[1].Name, "Activity 1")
	assert.Equal(t, "a1", acts[2].Name, "Activity 2")
	assert.Equal(t, "p", acts[3].Name, "Activity 3")
}

func validatePipelineID(t *testing.T, pID activities.PipelineID, expectedID string, expectedName string) {
